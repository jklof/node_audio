...
class RMVPEF0EstimatorNodeItem(NodeItem):
    ...
    def updateFromLogic(self):
        ...
        if model_path:
            self.model_path_label.setText(f"Model: ...{model_path[-30:]}")
        else:
            self.model_path_label.setText("Model: Not loaded")
        ...
        if f0 > 0:
            self.f0_label.setText(f"F0: {f0:.1f} Hz")
        else:
                ...
                try:
                    ...
                    with self._lock:
                        ...
                        audio_chunk_non_contiguous = np.array(
                            [self._audio_deque.popleft() for _ in range(REQUIRED_INPUT_SAMPLES)]
                        )
