...
from PySide6.QtWidgets import (
    QWidget, QLabel, QVBoxLayout, QPushButton, QComboBox, QFileDialog, QHBoxLayout, QSpinBox
)
...
class RVCOnnxInferenceNodeItem(NodeItem):
    ...
    def __init__(self, node_logic: "RVCOnnxInferenceNode"):
        ...
        device_v_layout = QVBoxLayout(); device_v_layout.addWidget(QLabel("Device:"))
        self.device_combo = QComboBox()
        device_v_layout.addWidget(self.device_combo); controls_layout.addLayout(device_v_layout)
        spk_v_layout = QVBoxLayout(); spk_v_layout.addWidget(QLabel("Speaker ID:"))
        self.spk_id_spin = QSpinBox(); self.spk_id_spin.setRange(0, 100)
        spk_v_layout.addWidget(self.spk_id_spin); controls_layout.addLayout(spk_v_layout)
        ...
        self.ui_updater = QTimer(self); self.ui_updater.setInterval(200)
    ...
    def _populate_devices(self):
        ...
        if "CUDAExecutionProvider" in onnxruntime.get_available_providers():
            ...
            except Exception as e: logger.warning(f"Could not enumerate CUDA devices for ONNX: {e}")
    ...
    def _on_load_model_clicked(self):
        ...
        if path: self.node_logic.set_model_path(path)
    ...
    def updateFromLogic(self):
        ...
        if is_strained:
            ...
            self.status_label.setStyleSheet("color: red;"); self.buffer_label.setStyleSheet("color: red;")
        else:
            self.status_label.setStyleSheet("color: white;"); self.buffer_label.setStyleSheet("color: gray;")
        self.status_label.setText(status_text)
        task_queue = state.get("task_queue_size", 0); output_buffer = state.get("output_buffer_size", 0)
        self.buffer_label.setText(f"Buffer: In {task_queue} | Out {output_buffer}")
        info = state.get("info", {}); info_text = (f"SR: {info.get('sr')} | F0: {info.get('f0')} | Half: {info.get('is_half')}" if info else "Info: Awaiting model")
...
class RVCOnnxInferenceNode(Node):
    ...
    def _load_model(self):
        with self._lock:
            ...
            self._status = "Idle"; self._is_half = False
        if not model_path or not os.path.exists(model_path):
            self._session, self._metadata = None, {}; return

        provider = "CPUExecutionProvider"; provider_options = {}
        if gpu >= 0 and "CUDAExecutionProvider" in onnxruntime.get_available_providers():
            provider = "CUDAExecutionProvider"; provider_options = {"device_id": str(gpu)}
        
        try:
            with self._lock: self._status = "Loading..."
            so = onnxruntime.SessionOptions(); so.log_severity_level = 3
            session = onnxruntime.InferenceSession(model_path, sess_options=so, providers=[provider], provider_options=[provider_options])
            meta = session.get_modelmeta()
            if "metadata" not in meta.custom_metadata_map: raise ValueError("ONNX model is missing metadata.")
            ...
            with self._lock: self._status = "Loaded"
            ...
            with self._lock: self._status = "Error"

    def _inference_loop(self):
        while not self._stop_event.is_set():
            task = None
            with self._lock:
                if self._tasks_deque: task = self._tasks_deque.popleft()
            ...
            if task and self._session:
                try:
                    ...
                    if model_sr != DEFAULT_SAMPLERATE:
                        ...
                        audio_out = resampy.resample(audio_out, sr_orig=model_sr, sr_new=DEFAULT_SAMPLERATE, filter='kaiser_fast')
    ...
    @Slot(str)
    def set_model_path(self, path: str): self._model_path = path; self._load_model()
    @Slot(int)
    def set_device(self, device_id: int): self._device = device_id; self._load_model()
    @Slot(int)
    def set_speaker_id(self, sid: int): self._speaker_id = sid
    ...
    def get_current_state_snapshot(self) -> Dict:
        with self._lock:
            ...
            return {"model_path": self._model_path, "device": self._device, "speaker_id": self._speaker_id,
                    "is_processing": self._is_processing, "is_strained": self._is_strained, "status": self._status,
    ...
    def process(self, input_data: dict) -> dict:
        ...
        if session_active and features is not None:
            if not is_f0_model or (f0_coarse is not None and pitchf is not None):
                ...
                if is_f0_model: min_len = min(min_len, len(f0_coarse), len(pitchf))
                ...
                if min_len > 0:
                    ...
                    with self._lock: self._tasks_deque.append(input_dict)
    ...
    def start(self):
        with self._lock:
            self._tasks_deque.clear(); self._results_deque.clear()
    ...
    def stop(self):
        ...
        if self._worker_thread: self._worker_thread.join(timeout=0.5)
