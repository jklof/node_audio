...
from node_helpers import managed_parameters, Parameter  # <-- IMPORT DECORATOR
...
# ==============================================================================
# 1. JIT-Compiled Gain Computer (Unchanged)
# ==============================================================================
@torch.jit.script
...
# ==============================================================================
# 2. UI Class for the Brickwall Limiter Node (Unchanged)
# ==============================================================================
...
# ==============================================================================
# 3. Logic Class for the Brickwall Limiter Node (REFACTORED)
# ==============================================================================
...
class BrickwallLimiterNode(Node):
    ...
    threshold_db = Parameter(default=-0.1, clip=(MIN_THRESHOLD_DB, MAX_THRESHOLD_DB))
    release_ms = Parameter(default=100.0, clip=(MIN_RELEASE_MS, MAX_RELEASE_MS), on_change="_mark_params_dirty")

    def __init__(self, name: str, node_id: Optional[str] = None):
        super().__init__(name, node_id)
        self.add_input("in", data_type=torch.Tensor)
        # Sockets now match the parameter keys for automatic updates
    ...
    def _mark_params_dirty(self):
        """Callback method for the decorator's on_change hook."""
    ...
    # --- BOILERPLATE REMOVED ---
    # The following methods are now automatically generated by the decorator:
    # - set_threshold_db(self, value: float)
    # - set_release_ms(self, value: float)
    ...
    def process(self, input_data: dict) -> dict:
        ...
        # --- REFACTORED: Simplified parameter updates ---
        # This single method call replaces the entire block of socket-checking logic.
        self._update_params_from_sockets(input_data)

        with torch.no_grad():
            with self._lock:
                # Copy managed parameters to local variables for processing
