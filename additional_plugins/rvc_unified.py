...
from PySide6.QtWidgets import (
    QWidget, QLabel, QVBoxLayout, QPushButton, QComboBox, QFileDialog, QHBoxLayout, QSpinBox, QDoubleSpinBox, QCheckBox
)
...
class RVC_Hubert:
    def __init__(self, model_path: str, gpu: int = 0):
        ...
        if gpu < 0 or not torch.cuda.is_available():
            self.device = torch.device("cpu"); self.is_half = False
        else:
    ...
    def encode(self, audio_16khz: np.ndarray, emb_output_layer=9, use_final_proj=True) -> np.ndarray:
        ...
        if use_final_proj: feats_out = self.model.final_proj(logits[0])
        else: feats_out = logits[0]
...
class RVCUnifiedNodeItem(NodeItem):
    ...
    def __init__(self, node_logic: "RVCUnifiedNode"):
        ...
        self.rvc_label = QLabel("RVC: Not loaded"); self.rvc_label.setStyleSheet("font-size: 9px; color: gray;")
        self.hubert_label = QLabel("HuBERT: Not loaded"); self.hubert_label.setStyleSheet("font-size: 9px; color: gray;")
        self.rmvpe_label = QLabel("RMVPE: Not loaded"); self.rmvpe_label.setStyleSheet("font-size: 9px; color: gray;")

        layout.addWidget(self.load_rvc_button); layout.addWidget(self.rvc_label)
        layout.addWidget(self.load_hubert_button); layout.addWidget(self.hubert_label)
        layout.addWidget(self.load_rmvpe_button); layout.addWidget(self.rmvpe_label)

        controls_layout = QHBoxLayout()
        device_layout = QVBoxLayout(); device_layout.addWidget(QLabel("Device:"))
        self.device_combo = QComboBox()
        device_layout.addWidget(self.device_combo); controls_layout.addLayout(device_layout)
        
        spk_layout = QVBoxLayout(); spk_layout.addWidget(QLabel("Speaker ID:"))
        self.spk_id_spin = QSpinBox(); self.spk_id_spin.setRange(0, 100)
        spk_layout.addWidget(self.spk_id_spin); controls_layout.addLayout(spk_layout)
        
        pitch_layout = QVBoxLayout(); pitch_layout.addWidget(QLabel("Pitch Shift:"))
        self.pitch_spin = QDoubleSpinBox(); self.pitch_spin.setRange(-24.0, 24.0); self.pitch_spin.setSingleStep(0.1)
        pitch_layout.addWidget(self.pitch_spin); controls_layout.addLayout(pitch_layout)
        ...
        self.vad_spin.setRange(0.0, 1.0); self.vad_spin.setSingleStep(0.001); self.vad_spin.setDecimals(4)
        self.vad_spin.setToolTip("Voice Activity Detection threshold.\nProcessing is skipped on silent chunks.")
        vad_controls_layout.addWidget(self.vad_spin); perf_layout.addLayout(vad_controls_layout)
        
        chunk_layout = QVBoxLayout(); chunk_layout.addWidget(QLabel("Chunk (ms):"))
        self.chunk_spin = QSpinBox()
        self.chunk_spin.setRange(40, 2000); self.chunk_spin.setSingleStep(20); self.chunk_spin.setValue(320)
        self.chunk_spin.setToolTip("Larger chunks reduce CPU overhead but increase latency.")
        chunk_layout.addWidget(self.chunk_spin); perf_layout.addLayout(chunk_layout)
        layout.addLayout(perf_layout)
        
        sola_layout = QHBoxLayout()
        crossfade_layout = QVBoxLayout(); crossfade_layout.addWidget(QLabel("Crossfade (ms):"))
        self.crossfade_spin = QSpinBox()
        self.crossfade_spin.setRange(0, 500); self.crossfade_spin.setSingleStep(10); self.crossfade_spin.setValue(100)
        self.crossfade_spin.setToolTip("Duration of the audio crossfade to hide seams between chunks.")
        crossfade_layout.addWidget(self.crossfade_spin); sola_layout.addLayout(crossfade_layout)
        
        sola_search_layout = QVBoxLayout(); sola_search_layout.addWidget(QLabel("SOLA Search (ms):"))
        self.sola_search_spin = QSpinBox()
        self.sola_search_spin.setRange(0, 100); self.sola_search_spin.setSingleStep(2); self.sola_search_spin.setValue(10)
        self.sola_search_spin.setToolTip("Additional time to search for the best audio overlap point.\nHelps reduce artifacts.")
        sola_search_layout.addWidget(self.sola_search_spin); sola_layout.addLayout(sola_search_layout)
        ...
        self.prebuffer_spin.setToolTip("Context audio given to the model before the main chunk (ms).\nHelps prevent artifacts at the start of chunks.")
        ...
        self.ui_updater = QTimer(self); self.ui_updater.setInterval(UI_UPDATE_INTERVAL_MS)
        self.ui_updater.timeout.connect(self.updateFromLogic); self.ui_updater.start()
        self.updateFromLogic()

    def _populate_devices(self):
        self.device_combo.clear(); self.device_combo.addItem("CPU", -1)
        if "CUDAExecutionProvider" in onnxruntime.get_available_providers():
            try:
                if torch.cuda.is_available():
                    for i in range(torch.cuda.device_count()): self.device_combo.addItem(f"GPU {i}", i)
    ...
    def _on_load_model_clicked(self, model_type: str):
        ...
        if path: self.node_logic.set_model_path(model_type, path)
    ...
    def updateFromLogic(self):
        ...
        set_label(self.rvc_label, 'rvc'); set_label(self.hubert_label, 'hubert'); set_label(self.rmvpe_label, 'rmvpe')
        
        with QSignalBlocker(self.device_combo): self.device_combo.setCurrentIndex(self.device_combo.findData(state.get("device", -1)))
        with QSignalBlocker(self.spk_id_spin): self.spk_id_spin.setValue(state.get("speaker_id", 0))
        with QSignalBlocker(self.pitch_spin): self.pitch_spin.setValue(state.get("pitch_shift", 0.0))
        with QSignalBlocker(self.vad_checkbox): self.vad_checkbox.setChecked(state.get("vad_enabled", True))
        with QSignalBlocker(self.vad_spin): self.vad_spin.setValue(state.get("silent_threshold", 0.001))
        with QSignalBlocker(self.chunk_spin): self.chunk_spin.setValue(state.get("chunk_size_ms", 320))
        with QSignalBlocker(self.crossfade_spin): self.crossfade_spin.setValue(state.get("crossfade_ms", 100))
        with QSignalBlocker(self.sola_search_spin): self.sola_search_spin.setValue(state.get("sola_search_ms", 10))
        with QSignalBlocker(self.prebuffer_spin): self.prebuffer_spin.setValue(state.get("extra_conversion_ms", 100)) # --- NEW ---
        
        is_strained = state.get("is_strained", False)
        status_text = "Status: Overloaded!" if is_strained else ("Status: Processing" if state.get("is_processing") else state.get("status", "Idle"))
        ...
        info = state.get("info", {}); info_text = (f"SR: {info.get('sr')} | F0: {info.get('f0')} | Half: {info.get('is_half')}" if info else "Info: Awaiting models")
...
class RVCUnifiedNode(Node):
    ...
    def __init__(self, name: str, node_id: Optional[str] = None):
        ...
        self._device: int = -1; self._speaker_id: int = 0; self._pitch_shift: float = 0.0
        self._vad_enabled: bool = True; self._silent_threshold: float = 0.001
        self._chunk_size_ms: int = 320
        self._crossfade_ms: int = 100
        self._sola_search_ms: int = 10
        self._extra_conversion_ms: int = 100
        self._metadata: Dict[str, Any] = {}; self._is_half: bool = False
        self._model_sr: int = DEFAULT_SAMPLERATE
        self.f0_min = 50.0; self.f0_max = 1100.0
        self.f0_mel_min = 1127 * np.log(1 + self.f0_min / 700)
        self.f0_mel_max = 1127 * np.log(1 + self.f0_max / 700)
        
        self._status: str = "Idle"; self._is_processing = False; self._is_strained = False
    ...
    def _load_all_models(self):
        with self._lock:
            rvc_path, hubert_path, rmvpe_path = self._model_paths['rvc'], self._model_paths['hubert'], self._model_paths['rmvpe']
            gpu = self._device
        ...
        try:
            rvc_session = load_onnx(rvc_path, "RVC"); rmvpe_session = load_onnx(rmvpe_path, "RMVPE")
            hubert_model = None
            ...
            if rvc_session:
                ...
                if "metadata" in meta.custom_metadata_map:
                    ...
                    if not model_sr: raise ValueError("ONNX model metadata is missing 'samplingRate'.")
                else: raise ValueError("ONNX model is missing required metadata.")
            ...
            with self._lock: self._status = "Error loading models"
    ...
    def _generate_strength_curves(self):
        with self._lock:
            ...
            if self._last_crossfade_ms == self._crossfade_ms: return
                ...
                try:
                    ...
                    if len(stable_audio_out_model_sr) == 0:
                        ...
                        stable_audio_out = resampy.resample(
                            ...
                            sr_orig=model_sr, sr_new=DEFAULT_SAMPLERATE, filter='kaiser_fast'
                        )
                    else:
    ...
    def _perform_sola_stitching(self, stable_audio_out: np.ndarray, chunk_samples: int, crossfade_samples: int, sola_search_samples: int):
        with self._lock:
            if self._sola_buffer is not None:
                ...
                padded_slice = np.pad(sola_slice, (0, crossfade_samples - len(sola_slice)), 'constant') if len(sola_slice) < crossfade_samples else sola_slice[:crossfade_samples]
    ...
    def set_model_path(self, model_type: str, path: str):
        with self._lock: self._model_paths[model_type] = path
        self._load_all_models()
    @Slot(int)
    def set_device(self, device_id: int):
        with self._lock: self._device = device_id
        self._load_all_models()
    @Slot(int)
    def set_speaker_id(self, sid: int):
        with self._lock: self._speaker_id = sid
    @Slot(float)
    def set_pitch_shift(self, pitch: float):
        with self._lock: self._pitch_shift = pitch
    @Slot(bool)
    def set_vad_enabled(self, enabled: bool):
        with self._lock: self._vad_enabled = enabled
    @Slot(float)
    def set_silent_threshold(self, threshold: float):
        with self._lock: self._silent_threshold = threshold
    @Slot(int)
    def set_chunk_size_ms(self, ms: int):
        with self._lock: self._chunk_size_ms = ms
    @Slot(int)
    def set_crossfade_ms(self, ms: int):
        with self._lock: self._crossfade_ms = ms
        self._generate_strength_curves()
    @Slot(int)
    def set_sola_search_ms(self, ms: int):
        with self._lock: self._sola_search_ms = ms
    @Slot(int)
    def set_extra_conversion_ms(self, ms: int):
        with self._lock: self._extra_conversion_ms = ms

    def get_current_state_snapshot(self) -> Dict:
        with self._lock:
            info = {"sr": self._model_sr, "f0": self._metadata.get('f0', 1) == 1, "is_half": self._is_half} if self._are_all_models_loaded() else None
            return {
                "rvc_path": self._model_paths['rvc'], "hubert_path": self._model_paths['hubert'],
                "rmvpe_path": self._model_paths['rmvpe'], "device": self._device,
                "speaker_id": self._speaker_id, "pitch_shift": self._pitch_shift,
                "vad_enabled": self._vad_enabled, "silent_threshold": self._silent_threshold, 
                "chunk_size_ms": self._chunk_size_ms, "crossfade_ms": self._crossfade_ms,
                "sola_search_ms": self._sola_search_ms, "extra_conversion_ms": self._extra_conversion_ms,
                "is_processing": self._is_processing, "is_strained": self._is_strained, "status": self._status, "info": info,
    ...
    def process(self, input_data: dict) -> dict:
        ...
        with self._lock:
            ...
            if len(self._output_buffer) >= DEFAULT_BLOCKSIZE:
                ...
                if self._is_processing: logger.debug(f"[{self.name}] Buffer underrun, repeating last block.")
    ...
    def start(self):
        with self._lock:
            ...
            self._sola_buffer = None
            self._last_valid_output = np.zeros(DEFAULT_BLOCKSIZE, dtype=DEFAULT_DTYPE)
            self._is_processing = True; self._is_strained = False
    ...
    def stop(self):
        ...
        with self._lock: self._is_processing = False

    def remove(self): self.stop(); super().remove()

    def serialize_extra(self) -> dict:
        with self._lock:
            return { "rvc_model_path": self._model_paths['rvc'], "hubert_model_path": self._model_paths['hubert'],
                "rmvpe_model_path": self._model_paths['rmvpe'], "device": self._device,
                "speaker_id": self._speaker_id, "pitch_shift": self._pitch_shift,
                "vad_enabled": self._vad_enabled, "silent_threshold": self._silent_threshold, 
                "chunk_size_ms": self._chunk_size_ms, "crossfade_ms": self._crossfade_ms,
                "sola_search_ms": self._sola_search_ms, "extra_conversion_ms": self._extra_conversion_ms,
    ...
    def deserialize_extra(self, data: dict):
        ...
        self._device = data.get("device", -1); self._speaker_id = data.get("speaker_id", 0)
