...
class RVCContentEncoderNodeItem(NodeItem):
    ...
    def __init__(self, node_logic: "RVCContentEncoderNode"):
        ...
        if not RVC_HUBERT_DEPS_AVAILABLE:
            ...
            self.load_button.setEnabled(False); self.device_combo.setEnabled(False); self.version_combo.setEnabled(False)
    ...
    def _populate_devices(self):
        ...
        if not RVC_HUBERT_DEPS_AVAILABLE: return
    ...
    def _on_version_change(self, index: int):
        if index != -1: self.node_logic.set_rvc_version(self.version_combo.itemText(index))
    ...
    def updateFromLogic(self):
        ...
        with QSignalBlocker(self.version_combo):
            ...
            if index != -1: self.version_combo.setCurrentIndex(index)
...
class RVCContentEncoderNode(Node):
    ...
    def __init__(self, name: str, node_id: Optional[str] = None):
        ...
        self.add_input("in", data_type=np.ndarray); self.add_output("features", data_type=np.ndarray)
        self._lock = threading.Lock()
        
        self._model_path: Optional[str] = None; self._device: int = -1
        self._rvc_version: str = "v1"; self._encoder_instance: Optional[RVCContentEncoder] = None
        self._is_processing = False; self._last_proc_time_ms: float = 0.0
        self._is_strained: bool = False # For UI feedback

        self._audio_deque = collections.deque(); self._results_deque = collections.deque(maxlen=10)
        self._stop_event = threading.Event(); self._worker_thread: Optional[threading.Thread] = None

    def _process_audio_chunk(self, audio_chunk: np.ndarray, encoder: RVCContentEncoder, version: str = "v1") -> np.ndarray:
    ...
    def _encoder_loop(self):
        ...
        while not self._stop_event.is_set():
            with self._lock:
                encoder = self._encoder_instance; current_deque_size = len(self._audio_deque)
            ...
            if has_enough_data and encoder:
                ...
                if current_deque_size > WORKER_CHUNK_SAMPLES * MAX_BUFFER_CHUNKS:
                    ...
                    with self._lock:
                        ...
                        for _ in range(samples_to_discard): self._audio_deque.popleft()
                        logger.warning(f"[{self.name}] Buffer overflow. Discarding {samples_to_discard} old samples.")

                with self._lock: self._is_strained = is_strained_now # Update shared state for UI

                try:
                    with self._lock:
                        # Create the potentially non-contiguous array from the deque
                        audio_chunk_non_contiguous = np.array([self._audio_deque.popleft() for _ in range(WORKER_CHUNK_SAMPLES)])
                    # Explicitly create a C-contiguous copy.
                    audio_chunk = np.ascontiguousarray(audio_chunk_non_contiguous, dtype=np.float32)

                    start_time = time.monotonic()
                    with self._lock: version = self._rvc_version
                    ...
                    with self._lock:
                        self._results_deque.append(features); self._last_proc_time_ms = proc_duration_ms
    ...
    def set_device(self, device_id: int):
        with self._lock:
            if self._is_processing or self._device == device_id: return
            self._device = device_id
        self._load_model()
    
    @Slot(str)
    def set_rvc_version(self, version: str):
        with self._lock: self._rvc_version = version

    def get_current_state_snapshot(self) -> Dict:
        with self._lock:
            return {
                "model_path": self._model_path, "device": self._device, "version": self._rvc_version,
                "is_processing": self._is_processing, "deque_size": len(self._audio_deque),
                "last_proc_time_ms": self._last_proc_time_ms, "is_strained": self._is_strained
            }
    ...
    def process(self, input_data: dict) -> dict:
        ...
        if audio_in is not None:
            ...
            with self._lock:
                if self._encoder_instance: self._audio_deque.extend(mono_signal.astype(np.float32).tolist())

        latest_features = None
        with self._lock:
            if self._results_deque: latest_features = self._results_deque.popleft()
        return {"features": latest_features}
    
    def start(self):
        super().start()
        with self._lock:
            self._audio_deque.clear(); self._results_deque.clear()
            self._is_processing = True; self._last_proc_time_ms = 0.0; self._is_strained = False
    ...
    def stop(self):
        ...
        if self._worker_thread: self._worker_thread.join(timeout=0.5); self._worker_thread = None
        with self._lock:
            self._is_processing = False; self._audio_deque.clear(); self._results_deque.clear()
        logger.info(f"[{self.name}] Stopped processing.")

    def remove(self):
        self.stop(); super().remove()
    ...
    def deserialize_extra(self, data: dict):
        self._device = data.get("device", -1); self._rvc_version = data.get("version", "v1")
        self._model_path = data.get("model_path")
        if self._model_path: self._load_model()
