...
def _jit_feedback_loop(
    input_signal: torch.Tensor, initial_state: torch.Tensor, damping_coeff: float
    ...
    def process(self, input_data: dict) -> dict:
        ...
        with self._lock:
            ...
            if damping_socket is not None:
                ...
                if abs(self._damping - new_damping) > 1e-6:
                    self._damping = new_damping
                    ui_update_needed = True
            if ui_update_needed:
        ...
        output_signal_high_rate = F.interpolate(
            output_signal_low_rate.view(1, 1, -1), size=DEFAULT_BLOCKSIZE, mode="linear", align_corners=False
        ).squeeze()
