...
class MIDIKeyboardNode(Node):
    ...
    def __init__(self, name: str, node_id: Optional[str] = None):
        ...
        self._message_queue = deque(maxlen=100)  # For UI-generated messages
        self._active_notes = set()  # Single source of truth for note state
    ...
    def play_note(self, note: int, velocity: int):
        """Handles note-on events from the UI widget."""
        state_to_emit = None
        with self._lock:
    ...
    def stop_note(self, note: int):
        """Handles note-off events from the UI widget."""
        state_to_emit = None
        with self._lock:
    ...
    def process(self, input_data: Dict) -> Dict:
        """
        Handles incoming MIDI, updates state, and passes messages through.
        Also sends messages generated by the UI.
        """
        external_msg = input_data.get("msg_in")
        state_changed = False
        state_to_emit = None

        # --- Priority 1: Process and pass through external messages ---
        if isinstance(external_msg, mido.Message):
            with self._lock:
                if external_msg.type == "note_on" and external_msg.velocity > 0:
                    if external_msg.note not in self._active_notes:
                        self._active_notes.add(external_msg.note)
                        state_changed = True
                elif external_msg.type == "note_off" or (external_msg.type == "note_on" and external_msg.velocity == 0):
                    if external_msg.note in self._active_notes:
                        self._active_notes.discard(external_msg.note)
                        state_changed = True
            ...
            return {"msg_out": external_msg}

        # --- Priority 2: Process internal messages from the UI ---
        with self._lock:
