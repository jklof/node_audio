...
public:
    ...
    void set_parameters(const char *nam_file, float sample_rate, int max_buffer_size) {
        ...
        try {
            ...
            _is_loaded = false;
            _dsp.reset();
            // In a real plugin, you would save this error message to be read by Python
        }
    }

    void process(float** in_channels, float** out_channels, int num_channels, int num_samples) {
        // If not loaded, pass through dry/silence
        if (!_is_loaded || !_dsp || !in_channels || !out_channels || num_samples <= 0 || num_channels < 1) {
             for (int ch = 0; ch < num_channels; ++ch) {
                if (out_channels[ch]) {
                    // For safety, pass through the dry signal if possible (out_channels[ch] = in_channels[ch])
                    // But in this block, we only guarantee silence on error.
                    std::fill(out_channels[ch], out_channels[ch] + num_samples, 0.0f);
                }
            }
            return;
        }
        ...
        // The NAM DSP Core (DSP::process) expects NAM_SAMPLE*, which is 'float*'
        // due to the CMake fix (target_compile_definitions(nam_processor PRIVATE NAM_SAMPLE_FLOAT)).

        // 1. Process: Use the first input channel as the source and destination.
        //    NAM models are mono-input by design.
        NAM_SAMPLE* input_signal = (NAM_SAMPLE*)in_channels[0];
        NAM_SAMPLE* output_signal = (NAM_SAMPLE*)out_channels[0];

        try {
            _dsp->process(input_signal, output_signal, num_samples);
            
            // 2. Handle Stereo Output: Copy the mono result to all other channels
            for (int ch = 1; ch < num_channels; ++ch) {
                if (out_channels[ch]) {
                    std::copy(out_channels[0], out_channels[0] + num_samples, out_channels[ch]);
                }
            }
        } catch (const std::exception& e) {
            ...
            _is_loaded = false;
            _dsp.reset();
            // The Python code will read the error state and handle it.
        }
    }
...
PLUGIN_API void process_block(void* handle, float** in_channels, float** out_channels,
    ...
    if (handle) {
        static_cast<NamProcessor*>(handle)->process(in_channels, out_channels,
